...
function getFoods(callback) {
    ...
    let sql = "select f.id,f.food_name,f.photo, f.enabled, ft.food_type_desc_la, cc.curr_name_la,ks.kitchen_name,ks.kitchen_code, f.enabled_subtype, pm.cost, pm.price from foods f , food_types ft , currency_code cc , kitchens ks , price_masters pm where f.food_type_id = ft.id and cc.curr_code = f.currcode and f.kid = ks.id and pm.food_id = f.id group by f.food_name ";
